...
export default function HomePage() {
  ...
  // State cho "Xem Thêm" - Bắt đầu với 12 sản phẩm
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('📦 Total products from API:', data.featured?.length);
        setAllProducts(data.featured || []);
  ...
  useEffect(() => {
    ...
    switch(activeFilter) {
      ...
      case 'promotion':
        ...
        break;
      case 'new':
        filtered = filtered.filter(p => p.isNew).length > 0 
    ...
    setVisibleCount(4); // Reset về 12 sản phẩm khi đổi filter
